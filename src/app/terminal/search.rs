...
impl AxShell {
    ...
    pub(crate) fn open_search(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        let search_input = self.search.input.clone();
    ...
    pub(crate) fn perform_search(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        let query = self.search.input.read(cx).text().to_string();
        if query.is_empty() {
            self.search.query.clear();
            self.search.matches.clear();
            self.search.current = 0;
        ...
        self.search.target_tab = Some(tab.id.clone());
        ...
        self.status = format!(
            ...
            if match_count == 0 {
                ...
                format!("{}/{}", self.search.current + 1, match_count)
            }
        )
        .into();
    ...
    pub(crate) fn search_goto_next(&mut self, cx: &mut Context<Self>) {
        let match_count = count_match_groups(&self.search.matches);
        if match_count == 0 {
            return;
        }
        self.search.current = (self.search.current + 1) % match_count;
        self.jump_to_current_match(cx);
        cx.notify();
    }
    ...
    pub(crate) fn search_goto_prev(&mut self, cx: &mut Context<Self>) {
        ...
        self.search.current = (self.search.current + match_count - 1) % match_count;
        self.jump_to_current_match(cx);
        cx.notify();
    }
    ...
    fn jump_to_current_match(&mut self, _cx: &mut Context<Self>) {
        ...
        let Some((target_grid_line, _)) =
            find_nth_match_start(&self.search.matches, self.search.current)
        else {
            return;
        };
    ...
    pub(crate) fn search_highlight_map(
        ...
        if self.search.matches.is_empty()
            ...
            || self.search.target_tab.as_deref() != Some(tab_id)
        {
            return None;
        }
        ...
        let mut sorted: Vec<(i32, i32)> = self.search.matches.clone();
        sorted.sort();

        let mut group_idx = 0;
        let mut i = 0;
        while i < sorted.len() {
            let is_current = group_idx == self.search.current;
        ...
        div()
            ...
            .on_prepaint(move |bounds, _window, cx| {
                let _ = view.update(cx, |this, _| {
                    this.search.bar_bounds = Some(bounds);
                });
            })
            .on_mouse_down(
                MouseButton::Left,
                ...
                h_flex()
                    ...
                    .child(
                        div()
                            ...
                            .child(Input::new(&self.search.input).small()),
                    )
