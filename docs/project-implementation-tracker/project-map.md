...
| `src/app.rs` | 应用壳入口，声明 app 子模块、`AxShell` 状态结构和 type re-export | 新增/调整应用级状态字段、输入实体、scroll handle、runtime/event channel、workspace group 实例号、窗口初始化种类、Settings generation、saved context menu state、模块出口或跨模块共享类型时 | `AxShellWindowKind` 在构造期区分主/独立窗口；现代 Rust 具名入口，不再使用 `src/app/mod.rs` |
...
| `src/events.rs` | backend、SFTP、监控和同步共用的有界应用事件总线 | 改事件载荷、发送端类型、队列容量、资源事件路由或 app event loop 接线时 | 256 条 Tokio channel；`BackendEventSender` 通过 tab/group id 将既有 backend sender 路由到当前所有窗口；远程监控和 SSH 恢复检查事件携带 generation；SFTP 下载文件以 `TransferFileStarted` / `TransferFileFinished` 附着于批量 transfer；受管远端编辑以 `SftpEdit*` 事件回传工作副本状态；terminal `Output` 由 event loop 在连续输出段中批处理 |
| `src/diagnostics.rs` | 跨模块日志脱敏与诊断 helper | 改主机、用户、路径、错误链已知敏感值脱敏或统一诊断字段时 | 不得记录密码、私钥内容、token 或终端输入输出 |
| `src/monitoring.rs` | 本地系统采样、远端采样模型和格式化 | 改 CPU/MEM/NET/DISK 采样、远端 key/value 解析或字节格式化时 | 本地 sampler 只刷新 CPU/内存，不保留全进程表；原 `src/system.rs`，内容限定为监控领域 |
| `src/app/sftp.rs` | app 层每个连接组的 SFTP 页面状态 | 改当前远端路径、首次成功列举标记、分页状态、选择集、预览、受管编辑副本或 home dir 状态时 | `has_opened_directory` 区分初始占位值与已成功打开的远端目录，供空闲连接重建时恢复；`edit_sessions` 随 SFTP group 迁移，记录本地工作副本、修改和上传状态；`connection_may_be_stale` 让空闲连接在系统恢复后按需重建；与 `src/sftp/` 协议/传输实现分离 |
...
| `src/session.rs` | SSH、串口与 Telnet 会话领域模型 | 改 `Session`、`SessionKind`、`AuthMethod`、`SshConnectionMode`、串口参数、会话级 `sftp_path` / `shortcut`、legacy 或本地输入优化选择时 | 旧会话默认 `Ssh`；legacy 算法和高延迟本地输入优化均必须由会话显式开启；串口保存端口、波特率、数据位、校验、停止位和流控；仅 SSH 支持 SFTP；类型直接由 `crate::session` 导出 |
...
| `src/app/actions/session.rs` | 会话连接、统一 SSH/串口/Telnet 表单、local shell profile、tab 生命周期和工作区窗口迁移动作 | `save_session_from_form`，`set_session_kind`，`refresh_available_serial_ports`，`open_session`，`open_saved_session_sftp_only`，`retry_disconnected_tab`，`shutdown_all_backends`，`active_snapshot` | 改会话类型、端口默认值、串口端口检测、SSH 高级项、raw tab 创建、动态快捷键、断线重试或关闭 tab/group 时；Serial/Telnet 不创建 SFTP 或 SSH password prompt |
| `src/app/actions/pane.rs` | pane tree 操作和 group activation action | `split_current_pane`，`focus_adjacent_pane`，`activate_group_page`，`focus_pane_with_id`，`sync_system_tab_to_active_group` | Local split 复制 `LocalShellProfile`；Serial split 打开独立设备句柄，Telnet split 建立独立 TCP 连接 |
| `src/app/actions/saved_sessions.rs` | session selector、saved group、saved session 菜单和无凭据 share 导入/导出 action | `selector_entries`，`on_selector_key_down`，`saved_session_groups`，`session_tooltip_info`，`copy_saved_session_json`，`import_ssh_session_from_clipboard`，`export_saved_sessions_share_file`，`import_saved_sessions_share_file` | share schema v2 保存 Serial/Telnet 字段但不保存密码、私钥或代理密码；v1 兼容回退 SSH |
| `src/app/actions/sftp.rs` | UI 侧 SFTP 与本地文件浏览 action | `ensure_sftp_handle_for_group`，`ensure_sftp_local_resources_loaded`，`restore_active_local_sftp_path`，`sftp_worker_initial_request`，`open_sftp_at_terminal_working_dir`，`release_sftp_handle_for_group`，`pause_sftp_transfers_in_tab` | 仅从 SSH terminal/session 构建 SFTP；首次实际进入 SFTP 时才加载 file-icon cache 并枚举本地目录，之后远端/本地行只读内存图标；改按需建连、路径恢复、分页加载或 worker 回收/恢复时 |
| `src/app/actions/terminal.rs` | terminal 键盘、鼠标、修饰键、滚动、IME 和本地输入 overlay action | `on_terminal_modifiers_changed`，`on_terminal_key_down`，`send_terminal_input`，`paste_into_terminal`，`terminal_composition_for_tab`，`terminal_grid_point_and_side`，`on_terminal_scroll` | 固定 workspace/terminal 快捷键优先于保存 SSH 的动态连接快捷键；高延迟 overlay 只能处理明确安全的单行 ASCII，复杂输入必须 flush 后直通 |
| `src/app/terminal.rs` | app 侧 terminal 滚动、链接和短暂输入 UI 状态 | `TerminalScrollbarHandle`，`LocalInputBuffer`，链接激活 helper | 改 scroll handle、链接激活判定、会话内本地输入预测状态或其纯逻辑测试时 |
| `src/app/theme.rs` | 主题、内置字体注册、当前 theme profile 和 custom theme 逻辑 | `EMBEDDED_FONT_FAMILIES`，`BUILT_IN_FONT_FAMILIES`，`ensure_embedded_font_family_loaded`，`load_embedded_themes`，`apply_theme_profile` | app 视觉系统入口；内置字体按 family 首次使用注册，UI 配置在初始化前确保、terminal 在首个可见渲染前确保；增删字体需同步 `assets/fonts/README.md` 与 Settings 排序 |
| `assets/fonts/README.md` | 内置字体 family、版本、样式和用途清单 | Bundled Fonts 表、排除范围、授权入口 | 判断字体包中哪些文件需要编译进应用或核对内部 family 名时 |
| `src/app/lifecycle/startup.rs` | 启动辅助、进程环境、日志 writer/轮转、crash hook 和主 / 独立工作区窗口打开 | `configure_rayon_threads`，`init_logging`，`runtime_log_dir`，`crash_report_dir`，`open_main_window`，`open_workspace_window` | 主窗口以 `MainWorkspace` global 登记，并在匹配窗口关闭时移除；独立窗口以 `Detached` 初始化并接收已转移的 `TabGroup` 与事件 receiver，关闭时只关闭其拥有的 backend；启动期配置必须在 GPUI 初始化前应用 |
...
| `src/app.rs` | 全局 UI 状态结构和 app 子模块出口 | `AxShell` fields，`WorkspaceTransfer`，`MainWorkspace`，`AxShellWindowKind`，`FileIconCache`，rayon_threads_input，local shell profile inputs，type re-exports，saved session/group context menu state | 新增/调整应用级状态字段、跨窗口 workspace 所有权、窗口初始化种类、图标缓存、输入实体、scroll handle、runtime/event channel、Settings generation 或 saved sidebar 右键菜单状态时 |
| `src/app/lifecycle/init.rs` | `AxShell` 初始化、默认状态装配和独立工作区安装 | `AxShell::new`，`AxShell::new_with_events`，`AxShellWindowKind`，`install_workspace_transfer`，`ensure_embedded_font_family_loaded`，transfer history recovery，`backend_event_channel` | 所有窗口以空 file-icon cache / 本地 SFTP 浏览状态启动，首次 SFTP 才初始化；`Detached` 还跳过 transfer history clone，安装转移资源并关闭布局持久化；使用 `src/events.rs` 构造 256 条 Tokio backend event queue |
| `src/app/lifecycle/event_loop.rs` | 输入事件、后台事件分发、系统采样、主题同步和恢复兜底 | `on_input_event`，`start_event_pump`，`detect_system_resume`，`handle_system_resume`，`sample_system_if_due`，`drain_backend_events`，`TransferStarted` / `TransferProgress`，`flush_terminal_output` | 连续 `Output` 段按 tab 聚合；本地 `SystemSampler` 仅在监控实际可见时创建；不执行文件图标解析；长时间未调度仅恢复当前上下文，不自动重连 |
...
| `src/app/workspace.rs` | 工作区页面、tab/group 模型、会话组重排和同名实例号、连接进度、远程采样和布局持久化 | `TabGroup`，`WorkspacePage`，`workspace_tabs`，`next_workspace_group_instance`，`move_workspace_group_before`，`set_workspace_page`，`open_settings_page`，`open_about_page`，`request_active_ssh_resume_health_check`，`is_monitoring_visible` | 改 terminal/SFTP/settings tab、同名会话实例标识、会话组顺序、SFTP-only group 可见性和关闭回退、当前 tab 可见性、Settings 重开状态重置或指定分页跳转、页面关闭、连接重试、恢复健康检查或监控采样时；独立窗口永不报告监控可见 |
...
| `src/app/views/tab_bar.rs` | 顶部 tab bar、会话组拖放、同名实例标签、独立窗口入口和 split/search 操作按钮 | `render_tab_bar`，`WorkspaceTabDrag`，`tabs_scroll_handle` | 改编号 terminal/SFTP 标签、同名会话实例标识、会话组拖放重排、Terminal Tab 的“移到新窗口”入口、当前 tab 自动可见、SFTP 标签关闭、tab 选择/关闭、settings tab、split pane 按钮或 tab bar 搜索按钮时 |
...
| `src/terminal/element.rs` | terminal 行级 `ShapedLine` 缓存、字体 metrics、等宽保护与实时覆盖层绘制 | `TerminalElement`，`TerminalComposition`，`paint_composition`，`GridLayoutCache`，`GridLayoutKey`，`hovered_url_underlines`，`RowLayout` | 在 metrics / shape 前确保当前内置 terminal family 已注册；`GridLayoutKey` 只含影响 shaped row 的状态，IME/local overlay 在 paint 独立处理；改终端 text/background/block 准备缓存、搜索/keyword 色彩失效键、Command/Ctrl 下的 URL/path 下划线、光标/选择/IME/本地输入 overlay 或字体间距问题时 |
| `src/terminal.rs` | terminal 模块入口和公开出口 | module declarations，backend/tab/input exports | 改 terminal 模块树或既有 `crate::terminal::*` 路径时 |
| `src/terminal/backend.rs` | terminal backend command、sender 和 shutdown controller | `BackendCommand`，`BackendTx`，`BackendShutdown` | 统一 Local、SSH、Serial、Telnet command sender 和关闭控制；全应用事件在 `src/events.rs` |
| `src/terminal/tab.rs` | terminal tab、共享行块 snapshot、滚屏复用、selection、高亮节流和 SSH 输入反馈计时 | `TabKind`，`TerminalTab`，`InputFeedbackTracker`，`local_input_overlay_anchor`，`new_serial_or_telnet`，`send_user_input`，`RenderSnapshot`，`RenderRow`，`SnapshotCache` | Serial/Telnet 复用终端渲染与 scrollback；SSH 键盘/IME 输入仅匿名记录到首个后续输出的反馈时间，连续输入合并、30 秒超时丢弃、重连重置；local overlay gate 仅允许显式 opt-in 的主屏 SSH；`feed` / resize / scroll 累积并 reset damage |
...
- `rg -n 'local_input_optimization|LocalInputBuffer|local_input_overlay_anchor|TerminalComposition' src/session.rs src/app src/terminal locales`
...
- `rg -n 'SessionKind|serial_port|baud_rate|available_port_names|spawn_serial_terminal|spawn_telnet_terminal' src/session.rs src/backend src/app src/terminal`
