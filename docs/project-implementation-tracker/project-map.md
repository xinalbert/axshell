...
| `src/app/actions/sftp.rs` | UI 侧 SFTP 与本地文件浏览 action | `ensure_sftp_handle_for_group`，`sftp_worker_initial_request`，`sftp_worker_restore_path`，`open_sftp_entry_on_double_click`，`request_sftp_edit_uploads`，`open_sftp_at_terminal_working_dir`，`open_sftp_and_reveal_path`，`release_sftp_handle_for_group`，`pause_sftp_transfers_in_tab`，`sweep_idle_sftp_connections`，`mark_idle_sftp_connections_stale` | 仅从 SSH terminal/session 构建 SFTP；远端文件双击创建受管编辑副本，修改后由用户确认上传；改按需建连、明确路径直达、会话 `sftp_path`、分页加载或 worker 回收/恢复时 |
...
| `src/app/lifecycle/init.rs` | `AxShell` 初始化、默认状态装配和独立工作区安装 | `AxShell::new`，`AxShell::new_with_events`，`AxShellWindowKind`，`install_workspace_transfer`，`FileIconCache::load`，`start_file_icon_cache_refresh`，rayon_threads_input，transfer history recovery，`backend_event_channel` | 主窗口按需创建系统采样器；`Detached` 跳过 icon cache、配置本地目录验证/枚举和 icon refresh，安装转移资源并关闭布局持久化；使用 `src/events.rs` 构造 256 条 Tokio backend event queue |
...
| `src/terminal/tab.rs` | terminal tab、共享行块 snapshot、滚屏复用、selection 和高亮节流 | `TabKind`，`TerminalTab`，`new_serial_or_telnet`，`RenderSnapshot`，`RenderRow`，`SnapshotCache` | Serial/Telnet 复用终端渲染与 scrollback；`feed` / resize / scroll 累积并 reset damage |
